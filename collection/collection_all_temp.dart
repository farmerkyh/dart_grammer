...
void redeceFoidStudy() {
  ...
  //             next : numbers의 다음값
  final numSum = num.reduce((prev, next) {
    return prev + next;
  });
  ...
  //4.3 reduce함수는 return type이 List<String> 이므로 value는 String Type이여야 한다.
  //    그래서 아래는 오류임
  ...
  //----------------------------------------------------------------------
  final sum = num.fold<int>(0, (prev, next) {
    return prev + next;
  });
...
class Person {
  ...
  @override
  String toString() {
    return 'Person:$name, group:$group';
  }
}
